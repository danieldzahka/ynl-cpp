...
#define YNL_ARRAY_SIZE(array) \
  (sizeof(array) ? sizeof(array) / sizeof(array[0]) : 0)
...
typedef void* (*ynl_alloc_cb_t)(void* arg);

struct ynl_dump_no_alloc_state {
  struct ynl_parse_arg yarg;
  ynl_alloc_cb_t alloc_cb;
  ynl_parse_cb_t cb;
  __u32 rsp_cmd;
};
...
static inline void* ynl_nlmsg_data_offset(
...
static inline void* ynl_nlmsg_put_extra_header(
    struct nlmsghdr* nlh,
    unsigned int size) {
...
#define ynl_attr_for_each(attr, nlh, fixed_hdr_sz)                             \
  for ((attr) =                                                                \
           ynl_attr_first(nlh, (nlh)->nlmsg_len, NLMSG_HDRLEN + fixed_hdr_sz); \
       attr;                                                                   \
       (attr) = ynl_attr_next(ynl_nlmsg_end_addr(nlh), attr))

#define ynl_attr_for_each_nested(attr, outer)                                 \
  for ((attr) = ynl_attr_first(outer, outer->nla_len, sizeof(struct nlattr)); \
       attr;                                                                  \
...
static inline struct nlattr* ynl_attr_if_good(
    const void* end,
    struct nlattr* attr) {
...
static inline struct nlattr* ynl_attr_next(
    const void* end,
    const struct nlattr* prev) {
  struct nlattr* attr;
...
static inline bool __ynl_attr_put_overflow(struct nlmsghdr* nlh, size_t size) {
  bool o;
...
static inline struct nlattr* ynl_attr_nest_start(
    struct nlmsghdr* nlh,
    unsigned int attr_type) {
  struct nlattr* attr;

  if (__ynl_attr_put_overflow(nlh, 0))
    return (struct nlattr*)((char*)ynl_nlmsg_end_addr(nlh) - NLA_HDRLEN);
...
static inline void ynl_attr_nest_end(
    struct nlmsghdr* nlh,
    struct nlattr* attr) {
  attr->nla_len = (char*)ynl_nlmsg_end_addr(nlh) - (char*)attr;
}

static inline void ynl_attr_put(
    struct nlmsghdr* nlh,
    unsigned int attr_type,
    const void* value,
    size_t size) {
...
static inline void ynl_attr_put_str(
    struct nlmsghdr* nlh,
    unsigned int attr_type,
    const char* str) {
  struct nlattr* attr;
